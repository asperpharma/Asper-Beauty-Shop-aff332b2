...
import { ChevronRight, Loader2, Sparkles, Heart, Scissors, Droplets } from "lucide-react";
...
const FeaturedProductsCategory = ({ categorySlug, limit = 4 }: FeaturedProductsByCategoryProps) => {
  ...
  const bgClass = categorySlug === 'skin-care' ? 'bg-white' : 
                  categorySlug === 'make-up' ? 'bg-cream' : 'bg-white';
  ...
  return (
    <section className={`py-16 lg:py-24 ${bgClass} border-y border-gold/10 relative overflow-hidden`}>
        ...
        <div className="flex items-center justify-between mb-12 pb-6 border-b-2 border-gold/20">
          <div className="flex items-center gap-6">
            ...
            <div className="w-16 h-16 relative rounded-full bg-gradient-to-br from-gold/20 via-gold/10 to-transparent border-2 border-gold/40 flex items-center justify-center shadow-gold-badge hover:shadow-gold-badge-hover group/icon">
              ...
              <CategoryIcon className="w-7 h-7 text-gold relative z-10 transition-transform duration-500 group-hover/icon:scale-110" strokeWidth={2} />
            ...
            <div>
              ...
              <p className="font-body text-base text-muted-foreground max-w-2xl leading-relaxed">
                {isArabic ? categoryInfo.descriptionAr : categoryInfo.description}
              </p>
            </div>
          </div>
          <Link
