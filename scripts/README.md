...
The `audit-categories.ts` script analyzes all products in your Shopify store and
identifies category mismatches based on keyword analysis. It generates a
comprehensive report to help you organize your product catalog.
...
2. **Shopify Configuration**: Ensure your Shopify credentials are properly
   configured in `src/lib/shopify.ts`
...
1. **Fetches All Products**: Connects to your Shopify store and retrieves all
   products
2. **Analyzes Categories**: Uses keyword matching to detect the correct category
   for each product
...
2. Update product categories in Shopify Admin for products with high confidence
   scores (>70%)
